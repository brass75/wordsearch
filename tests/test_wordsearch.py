...
def patches(monkeypatch):
    monkeypatch.setattr('wordsearch.choice', lambda l: l[-1])
    monkeypatch.setattr('wordsearch.shuffle', lambda l: l)
    monkeypatch.setattr('wordsearch.randint', lambda *_, **__: 0)
...
class TestWordSearch:
    ...
    def test_grid_as_str(self, test_grid):
        assert wordsearch.grid_as_str(test_grid) == '''. . . . .
. . . . .
. . . . .
. . . . .
. . . . .'''

    @pytest.mark.parametrize('backwards, diagonal, grid_size, words, expected, output',
                             [
                                 (False, False, 5, ['GIRL', 'BOY', 'DOG'],
                                  ([['.', 'D', 'B', 'G', '.'],
                                    ['.', 'O', 'O', 'I', '.'],
                                    ['.', 'G', 'Y', 'R', '.'],
                                    ['.', '.', '.', 'L', '.'],
                                   ...
                                   ['GIRL', 'BOY', 'DOG']),
                                  ''),
                                 (False, False, 5, ['GIRL', 'BOY', 'DOG', 'TOOLONG'],
                                  ([['.', 'D', 'B', 'G', '.'],
                                    ['.', 'O', 'O', 'I', '.'],
                                    ['.', 'G', 'Y', 'R', '.'],
                                    ['.', '.', '.', 'L', '.'],
                                 ...
                                 (True, True, 5, ['GIRL', 'BOY', 'DOG', 'CHILD'],
                                  ([['.', '.', 'G', 'Y', 'D'],
                                    ['.', 'O', 'O', 'L', 'L'],
                                    ['D', 'B', 'I', 'R', '.'],
                                    ['.', 'H', 'I', '.', '.'],
    ...
    @pytest.mark.parametrize('direction, x, y, output, exception',
                             [
                                 ('v', 5, 5, (6, 5), False),
                                 ('h', 5, 5, (5, 6), False),
                                 ('du', 5, 5, (4, 6), False),
